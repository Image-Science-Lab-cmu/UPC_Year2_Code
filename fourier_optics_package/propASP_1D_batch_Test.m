...
%% Test 1: Box
[exp_x2, exp_u2] = propASP_1D(Uin_box, lambda, delta1, delta2, z);
[x2, u2] = propASP_1D_batch(Uin_box, lambda, delta1, delta2, z);
assert(sum(abs(exp_u2 - u2)) == 0);
assert(sum(abs(exp_x2 - x2)) == 0);

%% Test 2: square wave
[~, exp_u2] = propASP_1D(Uin_squareWave_1, lambda, delta1, delta2, z);
[~, u2] = propASP_1D_batch(Uin_squareWave_1, lambda, delta1, delta2, z);
assert(sum(abs(exp_u2 - u2)) == 0);

%% Test 3: square wave mini-batch same Uin
[exp_x2, exp_u2] = propASP_1D(Uin_squareWave_1, lambda, delta1, delta2, z);
[~, u2] = propASP_1D_batch(Uin_squareWave_5_same, lambda, delta1, delta2, z);
tol = 1e-10;
...
[~, u2] = propASP_1D_batch(Uin_squareWave_5_diff, lambda, delta1, delta2, z);

tol=1e-10;
[exp_x2, exp_u2] = propASP_1D(Uin_squareWave_5_diff(1,:), lambda, delta1, delta2, z);
assert(max(abs(exp_u2 - u2(1, :))) < tol, 'The first output error');

[exp_x2, exp_u2] = propASP_1D(Uin_squareWave_5_diff(2,:), lambda, delta1, delta2, z);
assert(max(abs(exp_u2 - u2(2, :))) < tol, 'The first output error');

[exp_x2, exp_u2] = propASP_1D(Uin_squareWave_5_diff(3,:), lambda, delta1, delta2, z);
assert(max(abs(exp_u2 - u2(3, :))) < tol, 'The first output error');

[exp_x2, exp_u2] = propASP_1D(Uin_squareWave_5_diff(4,:), lambda, delta1, delta2, z);
assert(max(abs(exp_u2 - u2(4, :))) < tol, 'The first output error');

[exp_x2, exp_u2] = propASP_1D(Uin_squareWave_5_diff(5,:), lambda, delta1, delta2, z);
assert(max(abs(exp_u2 - u2(5, :))) < tol, 'The first output error');

% conclusion: propASP_1D_batch functions the same as propASP_1D
